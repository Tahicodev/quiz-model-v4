 ...
 * Landing-page component for the student SPA shell (index.html).
 *
 * Paints the exact same DOM that used to live inline in index.html so the
 * coupled legacy scripts (script.js, auth.js, utils.js, landing.js) keep
 * finding every element by id/class. All inline `onclick` handlers are kept
 * verbatim — they resolve to the `window.*` globals the legacy bundle exposes
 * (closeLightbox, navigateLightbox, zoomLightboxImage, resetLightboxZoom,
 * handleStudentAuthClose, showStudentResults).
 *
 * Rendering uses a single template string + a detached <template> node so the
 * inline event-handler attributes in the markup continue to work. None of the
 * markup is user-generated, so innerHTML carries no XSS risk here.
 */

/**
 * Full landing markup, byte-faithful to the pre-migration index.html body.
 * Kept as one string so SVG paths and inline handlers stay diffable.
 */
const LANDING_HTML = `
...
/**
 * Inject HTML into a container while preserving inline event-handler
 * attributes (innerHTML assignment does this natively). Slightly safer than
 * assigning directly because we parse into a detached fragment first.
...
/**
 * Mount the student landing page into the given container.
 * @param {HTMLElement} container - the <main id="app"></main> shell host
 */
export function initStudentLanding(container) {
  if (!container) return;
