...
router.post('/reset-data', adminOnly, async (req, res, next) => {
  try {
    ...
    const refreshTokenModel = repo.modelFor ? repo.modelFor('refresh_tokens') : null;
    if (userModel && refreshTokenModel) {
      const removedUsers = await userModel.findMany({
        where: { school_id: schoolId, role: { in: [ROLES.STUDENT, ROLES.TEACHER] } },
        select: { id: true },
      });
    ...
    if (aiModel) {
      ...
      const orphans = allCfgs.filter((c) => c.owner_id && !keepIds.has(c.owner_id));
      if (orphans.length) {
        await aiModel.deleteMany({ where: { id: { in: orphans.map((c) => c.id) } } });
      }
    ...
    const gamificationModel = repo.modelFor ? repo.modelFor('gamification') : null;
