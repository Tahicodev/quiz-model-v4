...
describe('UserService', () => {
	...
	describe('create()', () => {
		...
		it('throws ConflictError when username is taken', async () => {
			...
			await expect(
				service.create(
					{ username: 'dupe', password: '123456', name: 'Test' },
					ADMIN,
				),
		...
		it('creates a user successfully', async () => {
			...
			const result = await service.create(
				{ username: 'newuser', password: '123456', name: 'New' },
				ADMIN,
			);
		...
		it('writes legacy compatibility fields so student workspace can still authenticate the new account', async () => {
			repo.getAll.mockResolvedValue({ data: [], total: 0 });
			repo.create.mockResolvedValue({
				id: 'u-new',
				username: 'student1',
	...
	describe('delete()', () => {
		it('throws when deleting own account', async () => {
			await expect(service.delete(ADMIN.id, ADMIN)).rejects.toBeInstanceOf(
				ValidationError,
			);
		});
		...
		it('throws when deleting the only admin', async () => {
			...
			repo.getAll.mockResolvedValue({
				data: [{ id: 'other-admin', role: ROLES.ADMIN }],
				total: 1,
			});
			await expect(service.delete('other-admin', ADMIN)).rejects.toBeInstanceOf(
				ValidationError,
			);
		});
	...
	describe('resetPassword()', () => {
		it('rejects short passwords', async () => {
			...
			await expect(
				service.resetPassword('u-3', 'abc', ADMIN),
