...
function updateQuizConnectionPositions() {
	...
	connections.forEach((connection) => {
		...
		if (leftItem && rightItem) {
			...
			// Calculate connection points with proper offset for arrow alignment
			const leftX = leftItemRect.right - leftRect.left;
			const leftY = leftItemRect.top - leftRect.top + leftItemRect.height / 2;
			const rightX = rightItemRect.left - leftRect.left;
			const rightY =
				rightItemRect.top - leftRect.top + rightItemRect.height / 2;
			...
			// Apply positioning with improved arrow alignment
			connection.style.width = length + 'px';
			connection.style.left = leftX + 'px';
			connection.style.top = leftY + 'px';
			connection.style.transform = `rotate(${angle}deg)`;
			...
			let arrow = connection.querySelector('.connection-arrow');
			if (!arrow) {
