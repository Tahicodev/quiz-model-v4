(function () {
	...
	function bootRealtimeClient() {
		...
		socket.on('connect_error', (error) => {
			...
			var isAuthFailure =
				/unauthorized|invalid.*token|expired|token.*required|jwt/i.test(
					message,
				);
		...
		function upsertPushedExam(sessionPackage) {
			...
			const questionIds = Array.isArray(sessionPackage.questions)
				? sessionPackage.questions
						.map((question) => question?.id || question?.questionId || question)
						.filter(Boolean)
				: [];
			const pushedExam = {
				...(existing || {}),
				...
				passingScore:
					sessionPackage.passingScore ?? existing?.passingScore ?? 60,
				questions: questionIds.length ? questionIds : existing?.questions || [],
			};
		...
		socket.on('admin:syncGames', (payload) => {
			...
			if (payload.quizGames) {
				...
				const mergeWarmup = (existingWarmup, incomingWarmup) => {
					...
					merged.startedAt =
						incomingWarmup.startedAt || existingWarmup.startedAt;
					return merged;
				};
				...
				const mergeCard = (existingCard, incomingCard) => {
					...
					if (
						Object.prototype.hasOwnProperty.call(incomingCard, 'pendingCard')
					) {
					...
					if (
						Object.prototype.hasOwnProperty.call(incomingCard, 'lastResult')
					) {
					...
					if (preserveAuthoritativeLiveSession) {
						...
						merged.session.participants =
							...
							existingSession.participants.length
								? existingSession.participants
								: merged.session.participants;
						merged.session.round =
							existingSession.round || merged.session.round;
							...
							const shouldAccept = payload.quizGames.some((incoming) => {
								...
								if (incomingWarmup && currentWarmup) {
									...
									const incomingAnswerCount = (incomingWarmup.answers || [])
										.length;
									const currentAnswerCount = (currentWarmup.answers || [])
										.length;
				...
				let finalGames = Array.isArray(payload.quizGames)
					? payload.quizGames
					: [];
		...
		socket.on('school:profile-updated', (payload = {}) => {
			if (payload?.profile?.name) {
				...
				if (
					window.Auth &&
					typeof window.Auth.refreshSchoolBranding === 'function'
				) {
